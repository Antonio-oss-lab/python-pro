...
def hello_world():
    ...
    <a href="/random_facts">Hecho Aleatorio sobre Dependencias Tecnológicas</a><br>
    <a href="/contrasenas">Contraseña aleatoria</a><br>
...
@app.route("/contrasenas")
def contrasenas_aleatorias():
    contrasena = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890!@#$%^&*()-_=+]}[{;:/?.,<>``~~"
    aleatorias = ""
    for i in range(8):
        aleatorias += random.choice(contrasena)
    return f""" <h1>CONTRASENAS ALEATORIAS<h1>
    <p>{aleatorias}</p>"""
